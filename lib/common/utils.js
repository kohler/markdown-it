...
function handleAttributes(info, token, allowNB) {
  var oldAttrs, bpos, pos, apos, apos2, quot, vpos, len, code,
      wantAttr, wantValue, wantJoin, issp;
  ...
  for (++pos; pos < len; ++pos) {
    ...
    if (issp(code)) {
      ...
      return info;
    } else if (code === 0x23/*#*/ && pos < len - 1 && isAttributeContinue(info.charCodeAt(pos + 1))) {
      apos = pos + 1;
      ...
      wantJoin = false;
      --pos;
    } else if (code === 0x2E/*.*/ && pos < len - 1 && isAttributeContinue(info.charCodeAt(pos + 1))) {
      apos = pos + 1;
      for (pos += 2; pos < len && isAttributeContinue(info.charCodeAt(pos)); ++pos) {
      }
      wantAttr = 'class';
      ...
      if (pos === len) {
        ...
        if (pos === len || (code !== 0x22/* " */ && code !== 0x27/* ' */)) {
          break;
        }
        quot = code;
        vpos = pos + 1;
